...
void BVH::build(std::vector<Triangle> &&triangles)
{
    ...
    if (triangles.empty())
    {
        std::cout << "BVHState total_node_count: " << state.total_node_count << '\n'
                  << "BVHState total_triangle_count: " << state.total_triangle_count << '\n'
                  << "BVHState leaf_node_count: " << state.leaf_node_count << '\n'
                  << "BVHState max_leaf_node_triangle_count: " << state.max_leaf_node_triangle_count << '\n';
        return;
    }
    ...
    state.total_triangle_count = root->triangles.size();
    root->updateBounds();
    root->depth = 1;
    recursiveSplit(root, state);
    nodes.reserve(state.total_node_count);
    ordered_triangles.reserve(state.total_triangle_count);
...
std::optional<HitInfo> BVH::intersect(const Ray &ray, float t_min, float t_max) const
{
    ...
    while (true) 
    {
        ...
        {
            auto triangle_iter = ordered_triangles.begin() + static_cast<size_t>(node.triangle_index);
...
void BVH::recursiveSplit(BVHTreeNode *node, BVHState &state)
{
        ...
        for (size_t i = 1; i <= bucket_count - 1; i++)
        {
            ...
            if (left_triangle_count != 0)
            {
                ...
                if (cost < min_cost)
                {
                    ...
                    min_child0_tirangle_count = left_triangle_count;
                    min_child1_tirangle_count = right_triangle_count;
                }
            }
    ...
    child0->triangles.reserve(min_child0_tirangle_count);
    child1->triangles.reserve(min_child1_tirangle_count);
...
size_t BVH::recursiveFlatten(BVHTreeNode *node)
{
    ...
    {
        nodes[idx].triangle_index = static_cast<int>(ordered_triangles.size());
        for (const auto &triangle : node->triangles)
        {
