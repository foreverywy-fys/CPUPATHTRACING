...
void SceneBVH::build(std::vector<ShapeInstance> &&instances)
{
    ...
    if (instances.empty())
    {
        std::cout << "SceneBVHState total_node_count: " << state.total_node_count << '\n'
                  << "SceneBVHState total_instance_count: " << state.total_instance_count << '\n'
                  << "SceneBVHState leaf_node_count: " << state.leaf_node_count << '\n'
                  << "SceneBVHState max_leaf_node_instance_count: " << state.max_leaf_node_instance_count << '\n';
        return;
    }
    ...
    state.total_instance_count = root->instances.size();
    root->updateBounds();
    root->depth = 1;
    recursiveSplit(root, state);
    nodes.reserve(state.total_node_count);
    ordered_instances.reserve(state.total_instance_count);
...
std::optional<HitInfo> SceneBVH::intersect(const Ray &ray, float t_min, float t_max) const
{
    ...
    while (true) 
    {
        ...
        if (node.instance_count == 0)
        {
            ...
            {
                *(ptr++) = static_cast<int>(current_node_index + 1);
                current_node_index = static_cast<size_t>(node.child1_index);
            }
            else 
            {
        ...
        {
            auto instance_iter = ordered_instances.begin() + static_cast<size_t>(node.instance_index);
...
void SceneBVH::recursiveSplit(SceneBVHTreeNode *node, SceneBVHState &state)
{
    ...
    Bounds min_child0_bounds {};
    Bounds min_child1_bounds {};
    size_t min_child0_tirangle_count = 0;
    size_t min_child1_tirangle_count = 0;
        ...
        for (size_t i = 1; i <= bucket_count - 1; i++)
        {
            ...
            if (left_instance_count != 0)
            {
                ...
                if (cost < min_cost)
                {
                    ...
                    min_child0_tirangle_count = left_instance_count;
                    min_child1_tirangle_count = right_instance_count;
                }
            }
    ...
    child0->instances.reserve(min_child0_tirangle_count);
    child1->instances.reserve(min_child1_tirangle_count);
...
size_t SceneBVH::recursiveFlatten(SceneBVHTreeNode *node)
{
    ...
    if (bvh_node.instance_count == 0)
    {
        ...
        nodes[idx].child1_index = static_cast<int>(recursiveFlatten(node->children[1]));
    }
    else 
    {
        nodes[idx].instance_index = static_cast<int>(ordered_instances.size());
        for (const auto &instance : node->instances)
        {
